...
import { Printer, Search, FileText, AlertCircle, ArrowLeft } from 'lucide-react';
import { format } from 'date-fns';
import { useMemo, useState } from 'react';

export default function PerDeadlineSubmissionsPage() {
  const { id: deadlineId } = useParams<{ id: string }>();
  const { user } = useAuth();
  const navigate = useNavigate();
  const { data: submissions, isLoading } = useTeacherSubmissions(deadlineId);
  const [search, setSearch] = useState('');

  // Filter submissions by student name/email
    ...
    <div className="max-w-4xl mx-auto py-10 px-4">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => navigate('/deadlines/submissions')}>
            <ArrowLeft className="h-4 w-4 mr-1" />
            Alle opdrachten
          </Button>
          <h1 className="text-2xl font-bold ml-4">Ingeleverde Opdrachten (Deadline)</h1>
        </div>
